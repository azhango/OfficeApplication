...
import pers.hua.oa.mapper.LeaveFormMapper;
import pers.hua.oa.mapper.NoticeMapper;
import pers.hua.oa.mapper.ProcessFlowMapper;
import pers.hua.oa.service.exception.LeaveFormException;
...
public class LeaveFormService {
    ...
    public LeaveForm createLeaveForm(LeaveForm form) {
        LeaveForm f = (LeaveForm) MybatisUtils.executeUpdate(sqlSession -> {
            ...
            if (employee.getLevel() < 7) {
                ...
                //请假单已提交消息
                String notice1 = String.format("您的请假单[%s-%s]已提交请等待上级审批.", sdf.format(form.getStartTime()), sdf.format(form.getEndTime()));
                noticeMapper.insert(new Notice(employee.getEmployeeId(), notice1));
                //通知审批消息
                String notice2 = String.format("%s-%s提起的请假申请[%s-%s],清尽快审批.", employee.getTitle(), employee.getName(), sdf.format(form.getStartTime()), sdf.format(form.getEndTime()));
    ...
    public void audit(Long formId, Long operatorId, String result, String reason) {
        MybatisUtils.executeUpdate(sqlSession -> {
            ...
            Employee operator = employeeService.selectById(operatorId); //获取上级编号
            Employee employee = employeeService.selectById(form.getEmployeeId()); //获取发起者编号
            ...
            if (process.getIsLast() == 1) {
                ...
                String notice1 = String.format("您的请假申请[%s-%s]%s%s已%s,审批意见:%s,审批流程已结束",
                        ...
                        operator.getTitle(), operator.getName(), setResult, reason);
                noticeMapper.insert(new Notice(employee.getEmployeeId(), notice1));
                String notice2 = String.format("%s-%s提起请假申请[%s-%s]您已%s,审批意见:%s,审批流程已结束", employee.getTitle(), employee.getName(),
                        sdf.format(form.getStartTime()), sdf.format(form.getEndTime()), setResult, reason);
